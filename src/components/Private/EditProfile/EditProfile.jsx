...
export const EditProfile = ({setProfileEditModal}) => {
    ...
    function handleSubmit(e) {
        ...
        if(!nameError && !lastNameError && !emailError && !passwordError) {
            axios.put(`http://localhost:3000/users/${me?.id}`, {
    ...
    return (
      <Modal show={setProfileEditModal}>
          <h2 className="text-4xl text-center text-slate-600 my-8">Edit your profile</h2>
          <form onSubmit={handleSubmit} className="flex flex-col gap-6 items-center justify-center w-full bg-red">
            <Input value={me?.firstname} name={"First name"} ref={nameRef} error={nameError} />
            <Input value={me?.lastname} name={"Last name"} ref={lastNameRef} error={lastNameError} />
            <Input value={me?.email} type={"email"} name={"Email"} ref={emailRef} error={emailError} icon={<MdOutlineAttachEmail className={iconStyles} />} />
            ...
            <Input value={me?.avatar} type={"url"} name={"Avatar url"} ref={urlRef} error={urlError} icon={<FaUserCircle className={iconStyles} />} />
